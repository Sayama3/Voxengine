...
namespace Voxymore::Core
{
	void LightComponent::DeserializeComponent(YAML::Node& node)
	{
		...
		m_LightType = (LightType)node["LightType"].as<uint8_t>();
	}
	...
	void LightComponent::SerializeComponent(YAML::Emitter& out)
	{
		...
		out << KEYVAL("LightType", (uint8_t)m_LightType);
	}
	...
	bool LightComponent::OnImGuiRender()
	{
		...
		changed |= ImGui::DragFloat("Intensity", &m_Intensity);
		changed |= ImGui::DragFloat("Range", &m_Range);
	...
	{
		return Light(m_Color, tc.GetPosition(), tc.GetForward(), m_Range, m_Intensity, m_LightType);
	}

	Light LightComponent::AsLight(const glm::vec3& position, const glm::vec3& direction) const
	{
		return Light(m_Color, position, direction, m_Range, m_Intensity, m_LightType);
	}
}// namespace Voxymore::Core